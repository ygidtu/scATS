...
atsmix = function(n_max_ats, # maximum number of ATS sites
  ...
  if (typeof(st_arr) == "list") {
    st_arr = abs(unlist(st_arr))
  }

  if (typeof(en_arr) == "list") {
    en_arr = abs(unlist(en_arr))
  }
  ...
  if(!is.null(debug_pdf)) {
    ...
    tryCatch({
      ...
      # plot(density(st_arr,bw='SJ'),col='red',xlim=c(0,max_val))
...
# set.seed(5)
# data = readRDS('/mnt/raid64/ATS/Personal/zhangyiming/CG/NHC2_jl_all/test_R/test.rds')
# print(data$utr)
# st_arr = -data$st_arr
# en_arr = -data$en_arr
...
# res = atsmix(n_max_ats = 3, n_min_ats=1, st_arr = st_arr , en_arr=en_arr, L = data$L,
#              mu_f=300, # fragment length mean
#              sigma_f=50, # fragment length standard deviation
...
# min_val = min(min(st_arr), min(en_arr))
# max_val = max(max(st_arr), max(en_arr))
# plot(density(st_arr),col='red',xlim=c(0,max_val))
# lines(density(en_arr),col='blue')
# # plot(density(st_arr,bw='SJ'),col='red',xlim=c(0,max_val))
# # lines(density(en_arr,bw='SJ'),col='blue')
# abline(v=res$alpha_arr,col='green')

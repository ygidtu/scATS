...
class APA:
    ...
    def __init__(self,
        ...
        self.r1_utr_st_arr = np.array(r1_utr_st_arr)
        self.r1_len_arr = np.array(r1_len_arr)
        self.min_theta = min(self.r1_len_arr)
        self.r2_len_arr = np.array(r2_len_arr)
        ...
        self.pre_alpha_arr = np.array(pre_alpha_arr)
        self.pre_beta_arr = np.array(pre_beta_arr)
        ...
        # logger.debug('init split_data')
        self.data_wins = self.__split_data()

        # logger.debug('split_data done')
    ...
    def __init_run(self):
        ...
        # logger.debug('First all_theta_lik_mat calc')
        for i in np.arange(self.n_all_theta):

            # logger.debug(f'{i} all_theta_lik_mat calc, {self.n_all_theta}')
        ...
        # print(self.all_theta_lik_mat.sum())
            # logger.debug(f'{i} all_theta_lik_mat calc, done')
        # logger.debug('First all_theta_lik_mat calc, done')

        # logger.debug('sec all_theta_lik_mat calc')
        if any(pa_inds):
        ...
        # logger.debug('sec all_theta_lik_mat calc, done')
    ...
    def __split_data(self):
        ...
        # TODO, change the Nadaraya–Watson kernel regression into python
        ...
        # ks_res = ks.NadarayaWatsonSmoother(smoothing_parameter=2).fit_transform(FDataGrid(np.array(coverage_cnt)))
        # plt.plot(ks_res_x, coverage_cnt)
        # plt.plot(ks_res_x, ks_res)
    ...
    def kernel_smooth(self, beta):
        ...
        if np.sum(self.all_theta_lik_mat) > 0:
            for i in range(n_bd_step):
                tmp_inds = np.arange(max(i - n_bd_step, 0), i + n_bd_step)
                ...
                try:
                    all_win_log_lik_mat[:, i] = np.log(np.sum(tmp_weight * self.all_theta_lik_mat[:, tmp_inds], axis=1))
                except IndexError as err:
                    logger.error(np.sum(tmp_weight * self.all_theta_lik_mat[:, tmp_inds]))
                    logger.error(tmp_inds)
                    exit(err)
    ...
    def inference(self):
        ...
        for k, v in res_2.items():
            if "float" in str(type(v)):
                ...
                res_3[k] = list(v)
            elif type(v).__module__ == pd.__name__:
                res_3[k] = list(v)
            else:
                if v is not None:
                    res_3[k] = float(v)

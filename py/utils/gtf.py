...
class GTFFeature:
    ...
    @property
    def start(self):
        return int(self._start) if self._start is not None else None

    @property
    def end(self):
        return int(self._end) if self._end is not None else None
    ...
    def __len__(self):
        if self._end is not None and self._start is not None:
            ...
            return length
        raise ValueError(f"start/end ({self._start}/{self._end}) is None")

    ...
    function init(self::APAData)
        ...
        if sum(pa_inds) > 0
            for i in findall(isequal(1), pa_inds)
                ...
                x_arr = self.r1_utr_st_arr[i]
                if !isa(x_arr, Array)
                    x_arr = [x_arr]
                end

                l_arr = self.r1_len_arr[i]
                if !isa(l_arr, Array)
                    l_arr = [l_arr]
                end

                r_arr = self.r2_len_arr[i]
                if !isa(r_arr, Array)
                    r_arr = [r_arr]
                end

                s_arr = self.polya_len_arr[i]
                if !isa(s_arr, Array)
                    s_arr = [s_arr]
                end
                ...
                # println(r_arr)
                # println(s_arr)
                # println(self.all_theta[tmp_ind])
                # println(self.pmf_s_dis_arr)
                # println(self.s_dis_arr)
                ...
                # println(res)
                # println(self.all_theta_lik_mat)

                if !isa(tmp_ind, Array)
                    tmp_ind = [tmp_ind]
                end

                self.all_theta_lik_mat[i, tmp_ind] = res
            end
        end

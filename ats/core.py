...
class AtsModel(object):
    def __init__(
        ...
        self.st_arr = [x for x in st_arr if 0 <= x < utr_length]
        ...
        assert self.n_frag == len(
            self.st_arr), f"self.n_frag ({self.n_frag}) != len(self.st_arr) (l{len(self.st_arr)});"
    ...
    def mstep(self, para, Z, k):
        ...
        tmp_beta = np.sqrt(
            np.sum(Z[:, k] * ((self.st_arr - alpha_arr[k]) ** 2)) / tmp_sumk)
    ...
    def em_algo(self, para, fixed_inference_flag=False):
        ...
        para.alpha_arr = np.rint(para.alpha_arr).astype(
    ...
    def sample_alpha(self, n_ats):
        ...
        # extend to include peaks in 0 or L-1
        x_arr = np.arange(-100, self.L + 100)
        ...
        if n_ats <= len(peaks):
            ...
            mu = np.random.choice(peaks, size=n_ats -
                                  len(peaks), replace=True, p=peaks_ws)
            mu = np.sort(np.concatenate((peaks, mu)))
            shift = np.rint(np.random.normal(
                loc=0, scale=5 * self.step_size, size=n_ats))
            return mu + shift
    ...
    def init_para(self, n_ats):
        ...
        beta_arr = np.random.choice(
            self.predef_beta_arr, size=n_ats, replace=True)
        ws = self.init_ws(n_ats)

        para = Parameters(title='Initial parameters',
                          alpha_arr=alpha_arr, beta_arr=beta_arr, ws=ws, L=self.L)
        return para
    ...
    def run(self) -> Optional[Parameters]:
        ...
        self.predef_beta_arr = np.arange(
            self.step_size, self.max_beta, self.step_size)

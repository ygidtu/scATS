...
write.table(
  temp[temp$CellType %in% c("CD14 mono", "CD16 mono"), c("FCells", "CellType")], "/mnt/raid64/ATS/Personal/zhangyiming/quant/R_sel/sashimi/mono_bc.txt", 
  quote = F, col.names = F, row.names = F, sep = "\t"
)

write.table(
  data.frame(
    cell = c("CD14 mono", "CD16 mono"), 
...
go = enrichGO(
  sapply(markers$gene[markers$comp == "myeloid - lymphoid"], function(x) {
    str_split(x, "\\|")[[1]][1]
  }),
  OrgDb = org.Hs.eg.db,
  keyType = "SYMBOL",
...
```{r}
go = enrichGO(
  sapply(markers$gene[markers$comp == "CD14 mono - CD16 mono"], function(x) {
    str_split(x, "\\|")[[1]][1]
  }),
  OrgDb = org.Hs.eg.db,
  keyType = "SYMBOL",

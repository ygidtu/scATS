...
def run(args):
	bamfile, chrom, cb_df, info, outfile = args
	...
	for df in res_lst:
		...
		df.to_csv(outfile, mode=md, header=True, compression='gzip')
		md ='a'

	# return res


def main(bamfile: str, cb_file: str, peakfile: str, outfile: str, n_jobs: int = 12, region_size: int = 200, chunks: int = 10000):
	...
	with open(peakfile) as ph:
		for peak in tqdm(ph, desc="Reading Peaks", total=7323382):
	...
	with Pool(n_jobs) as pool:
		try:
			cmds = []
			for chrom, info in tqdm(peak_region.items(), desc="assign tasks"):

				cmds += [[bamfile, chrom, cb_df, info[x: x + chunks], os.path.join(temp_dir, f"{chrom}_{x}.csv.gz")] for x in range(0, len(info), chunks)]

				# for s_t, strand in info:
				# 	cmds.append([bamfile, chrom, s_t, strand, cb_df])
					# res_lst.append(pool.apply_async(run, (arg,)))
			
			list(tqdm(pool.imap(run, cmds), total=len(cmds)))

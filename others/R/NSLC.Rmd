...
```{r}
obj <- readRDS("/mnt/raid64/ATS/Personal/zhangyiming/quant/NSLC/ICAS_obj_psi.rds")
...
write.table(
  data.frame(
    cell = c(
      "AT2", "Basal", "Fib", 
      "B", "CD4", "CD8", "DC", "Mast", "Mφ"
    ), 
    color = c(
      RColorBrewer::brewer.pal(9, "Blues")[c(3, 5, 7)],
      RColorBrewer::brewer.pal(9, "YlOrRd")[c(2, 3, 4, 6, 7, 8)]
    )
  ), 
...
## Immunage and non immune

temp = obj@colData
temp$CellClass = ifelse(temp$CellType %in% c("AT2", "Basal", "Fib"), "Non-immune", "Immune")
temp$FCells = paste("NSLC", temp$Cells, sep = "_")

write.table(
  temp[, c("FCells", "CellClass")], "/mnt/raid64/ATS/Personal/zhangyiming/quant/NSLC/celltype1_bc.txt", 
  quote = F, col.names = F, row.names = F, sep = "\t"
)

write.table(
  data.frame(
    cell = c("Non-immune", "Immune"), 
...
```{r}
suppressPackageStartupMessages(require(tsidx))

obj <- readRDS("/mnt/raid64/ATS/Personal/zhangyiming/quant/NSLC/seurat_obj_psi.rds")
...
trqwe::mcsaveRDS(tsi, "/mnt/raid64/ATS/Personal/zhangyiming/quant/NSLC/tsi.rds")
trqwe::mcsaveRDS(as.data.frame(tsi@tsData$Tau), "/mnt/raid64/ATS/Personal/zhangyiming/quant/NSLC/tau.rds")
```
